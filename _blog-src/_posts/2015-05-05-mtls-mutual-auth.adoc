...
digraph G {
    ...
    Application -> Gateway [label="Unidirectional Auth", dir="both"];
...
```ShellSession
keytool -import -keystore shared_trust_store.jks -file apimanCA.cer
-alias apimanCA
```
...
keytool –keystore gateway_ks.jks –certreq –alias gateway –keyalg rsa
–file gateway.csr

keytool –keystore service_a_ks.jks –genkey –alias service_a -keyalg rsa
keytool –keystore service_a_ks.jks –certreq –alias service_a –keyalg rsa
–file service_a.csr
```
...
It's easy peasy to set up the gateway, but you should be especially careful about what you twiddle with, as the security implications could be important. Let's edit `apiman.properties` on our gateway(s) with a few simple settings to test things out:
...
Here's a small Java example using Jetty to create a tiny service with mutual authentication enabled. We hook up our _keystore_, _truststore_ with respective hard-to-guess passwords, and set `setNeedClientAuth(true)`:
...
public static void main(String... args) throws Exception {
      ...
      sslContextFactory.setKeyStorePath("/Users/msavy/tmp/keys/service_ks.jks");
      sslContextFactory.setKeyStorePassword("password");
      sslContextFactory.setKeyManagerPassword("password");
      sslContextFactory.setTrustStorePath("/Users/msavy/tmp/keys/shared_trust_store.jks");
...
Restart everything, and you should be set to test it out.
