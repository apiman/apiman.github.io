...
One particularly useful change I'd like to highlight is that the Vert.x Gateway's API, when secured by Keycloak, now accepts a list of additionally accepted issuers using `allowed-users`, which is useful for users with more complex auth setups:
